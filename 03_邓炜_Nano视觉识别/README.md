...
3. `F:\nano` 是邓炜的 Nano 工作备份；其中的工程、引擎和运行日志不得被重新标注为 haimichenha 的优化原始材料。haimichenha 的历史优化线索应从本地 Git 分支/缓存中另行核对。
4. 当前归档的项目运行口径是 **YOLOv5 + TensorRT FP32 + 320×320**。阶段性 FP16 文件只说明开发中存在过对比试验，不能反向改写最终 FP32 配置。
