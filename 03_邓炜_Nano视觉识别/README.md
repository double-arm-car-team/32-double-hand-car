...
2. **haimichenha 随后通过 SSH 连接 Nano 做运行时帧率优化试验**：关注引擎、输入尺寸、后处理、显示/采集节拍和日志留存，不承担模型训练或最初部署。
3. 当前归档的项目运行口径是 **YOLOv5 + TensorRT FP32 + 320×320**。优化阶段保存的 FP16 诊断试验是历史性能实验，不能反向改写最终 FP32 配置，也不能混作邓炜的训练/部署成果。

详见[运行配置与证据说明](运行配置与证据说明.md)和[SSH 优化试验归档](evidence/haimichenha_ssh_optimization_20260713/README.md)。
...
- **保留边界：**目录中同时存有 FP16 引擎和试验脚本，例如 `detect_fast.py`；其中一部分为 haimichenha 在部署后进行的 SSH 运行时优化试验。它们**不是本项目最终运行配置**，不得据此把项目写成 FP16 或混合精度部署。
